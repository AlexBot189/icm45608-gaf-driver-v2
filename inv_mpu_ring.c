						...
						s32 *q, u64 t, s16 accur)
{
	...
	u8 buf[IIO_BUFFER_BYTES];
	int ii;
	u16 hdr = st->sensor_l[j].header;
	bool is_quat = ((hdr & ~WAKE_HDR) == SIXQUAT_HDR) ||
		       ((hdr & ~WAKE_HDR) == NINEQUAT_HDR) ||
		       ((hdr & ~WAKE_HDR) == PEDQUAT_HDR);
	int n_elem = is_quat ? 4 : 3;
	...
	for (ii = 0; ii < 2; ii++)
		memcpy(buf + 4 * ii, &q[ii + 1], sizeof(q[ii]));
	iio_push_to_buffers(indio_dev, buf);
	/* push element 3 for quaternions */
	if (n_elem == 4) {
		memcpy(buf, &q[3], sizeof(q[3]));
		memset(buf + 4, 0, 4);
		iio_push_to_buffers(indio_dev, buf);
	}
	inv_push_timestamp(indio_dev, t);
